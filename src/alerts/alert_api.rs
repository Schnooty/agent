...
use lettre::{SmtpClient, SendableEmail, Envelope, ClientSecurity, ClientTlsParameters, Transport};
...
pub trait AlertApi {
    fn send_msteams_msg(&self, alert: &models::MsTeamsAlertBody, payload: &AlertPayload) -> ApiFuture<()>;
    fn send_email(&self, alert: &models::EmailAlertBody, payload: &AlertPayload) -> ApiFuture<()>;
    fn send_webhook(&self, alert: &models::WebhookAlertBody, payload: &AlertPayload) -> ApiFuture<()>;
}
...
impl AlertApi for AlertApiImpl {
    fn send_msteams_msg(&self, _alert: &models::MsTeamsAlertBody, _payload: &AlertPayload) -> ApiFuture<()> {
        Box::pin(async { 
            Err(Error::new("MS Teams alert is not yet implemented"))
        })
    }
    ...
    fn send_email(&self, alert: &models::EmailAlertBody, payload: &AlertPayload) -> ApiFuture<()> {
        ...
        Box::pin(async move {
            let (subject, _body) = match payload.status.status {
                models::MonitorStatusIndicator::OK => (format!("[Schnooty] Monitor {} has recovered", payload.monitor_name), format!("The following monitor has recovered: {}\n", payload.monitor_name)),
                models::MonitorStatusIndicator::DOWN => (format!("[Schnooty] Monitor {} is DOWN", payload.monitor_name), format!("The following monitor is now DOWN (failing): {}\n", payload.monitor_name))
            };
            let mut email_body = String::new();
            let timestamp: DateTime<Utc> = payload.status.timestamp;

            email_body.push_str(&format!("The following monitor {}: {}\n\n", 
            ...
            let (recipients, from, username, password, host, port, tls_mode) = match (alert.recipients, alert.from, alert.username, alert.password, alert.host, alert.port, alert.tls_mode) {
                (Some(recipients), Some(from), Some(username), Some(password), Some(host), Some(port), Some(tls_mode)) => (recipients, from, username, password, host, port as u16, tls_mode),
                _ => {
                    error!("Error getting alert data. At least one property missing: from, username, password, host, tls_mode");
                    return Err(Error::new("Error getting alert data. The alert is misconfigured."));
                }
            };

            let message_id = format!("https://api.schnooty.com/{}/{}", from, Utc::now().timestamp().to_string());
            ...
            let client_result = match tls_mode {
                ...
                models::TlsMode::TLS => {
                    ...
                    SmtpClient::new(address, ClientSecurity::Required(ClientTlsParameters {
                        connector,
                        domain: host.clone()
                    }))
                }
                models::TlsMode::STARTTLS => {
                    ...
                    SmtpClient::new(address, ClientSecurity::Opportunistic(ClientTlsParameters {
                        connector,
                        domain: host.clone()
                    }))
                }
    ...
    fn send_webhook(&self, _alert: &models::WebhookAlertBody, _payload: &AlertPayload) -> ApiFuture<()> {
        Box::pin(async { 
            Ok(())
        })
    }
